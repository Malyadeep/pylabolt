...
@numba.njit
def computeFields(elements, mesh, u_old, rho_old):
    u_sq, u_err_sq = 0, 0
    v_sq, v_err_sq = 0, 0
    rho_sq, rho_err_sq = 0, 0
    for ind in range(mesh.Nx * mesh.Ny):
        elements[ind].computeFields()
    ...
    resU = np.sqrt(u_err_sq/(u_sq * mesh.Nx * mesh.Ny + 1e-8))
    resV = np.sqrt(v_err_sq/(v_sq * mesh.Nx * mesh.Ny + 1e-8))
    resRho = np.sqrt(rho_err_sq/(rho_sq * mesh.Nx * mesh.Ny + 1e-8))
...
def solver(simulation):
    ...
    for timeStep in range(simulation.startTime, simulation.endTime,
        ...
        stream(simulation.elements, simulation.lattice,
               simulation.mesh)
        simulation.setBoundaryFunc(simulation.elements)
