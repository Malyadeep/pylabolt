...
class initialFields:
    def __init__(self, Nx, Ny, precision):
        self.fieldList = ['u', 'rho', 'boundaryNode']
...
def initFields(internalFields, mesh, precision, rank, comm):
    ...
    if rank == 0:
        if velType == 'translational':
            setFields(fields, rho_initial, mesh, velType, velValue=u_initial)
        elif velType == 'rotational':
            setFields(fields, rho_initial, mesh, velType,
                      velOmega=omega_initial, x_ref=x_ref_idx)
